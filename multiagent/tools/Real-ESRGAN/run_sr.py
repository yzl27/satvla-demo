...
DEFAULTS = dict(
    # ---------- 路径参数 ----------
    input_path="/home/990ep/wcj_datasets/HRSC_png/100000000.png",      # 输入：单张图片 或 目录
    output_dir="results",    # 输出目录
    model_path=str((Path(__file__).resolve().parent / "weights" / "RealESRGAN_x4plus.pth")),          # 自定义权重（None 自动下载）
...
CONFIG = SimpleNamespace(**DEFAULTS)  # 可在外部覆盖属性
...
def run(cfg):
    ...
    img_list = [str(inp)] if inp.is_file() else sorted(glob.glob(str(inp / "*")))
    if not img_list:
        raise FileNotFoundError(f"在 {cfg.input_path} 未找到任何文件")
    ...
    for idx, img_path in enumerate(img_list):
        ...
        save_path = out_dir / f"{name}{suffix}.{ext_save}"
