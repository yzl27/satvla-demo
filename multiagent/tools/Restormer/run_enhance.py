...
out_dir = os.path.join(CONFIG["result_dir"], task)
tile = CONFIG["tile"]
tile_step = CONFIG["tile_overlap"]
...
with torch.no_grad():
    for file_ in tqdm(files, desc="Processing"):
        ...
        fname = os.path.splitext(os.path.basename(file_))[0] + ".png"
    ...
    print(f"\nRestored images saved in → {out_dir}\n")
