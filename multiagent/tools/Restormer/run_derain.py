...
############################################################
# 🔧 USER CONFIGURATION (去雨配置)
############################################################
DEFAULTS = dict(
    task="Deraining", 
    input_dir="/home/990ep/wcj_datasets/HRSC_png/100000000.png",
    result_dir="./outputs/derained/",
    weight_path=os.path.join(os.path.dirname(os.path.abspath(__file__)), "weights", "deraining.pth"),
    device="cuda",
    tile=None,
    tile_overlap=32
)
...
def main():
    ...
    parameters = {
        ...
        "bias": False,
        "LayerNorm_type": "WithBias",  # 去雨为 WithBias
